 ...
 *******************************************************************************/
package org.metaborg.spoofax.eclipse.util.ui;
...
public abstract class FilteringInfoPopup extends PopupDialog implements DisposeListener {
    ...
    /**
     * Creates a tree information control with the given shell as parent. The given styles are applied to the shell and
     * the tree widget.
     *
     * @param parent
     *            the parent shell
    ...
    public void setLocation(Point location) {
        /*
         * If the location is persisted, it gets managed by PopupDialog - fine. Otherwise, the location is computed in
         * Window#getInitialLocation, which will center it in the parent shell / main monitor, which is wrong for two
         * reasons: - we want to center over the editor / subject control, not the parent shell - the center is computed
         * via the initalSize, which may be also wrong since the size may have been updated since via min/max sizing of
         * AbstractInformationControlManager. In that case, override the location with the one computed by the manager.
         * Note that the call to constrainShellSize in PopupDialog.open will still ensure that the shell is entirely
         * visible.
         */
    ...
    /**
     * Returns the name of the given element used for matching. The default implementation gets the name from the tree
     * viewer's label provider. Subclasses may override.
     * 
     * @param element
     *            the element
